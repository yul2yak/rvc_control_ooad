...
description: >-
  docs/OOA/01-System-Requirements.md의 FR·NFR을 근거로 Use Case와 SSD를
  작성한다. docs/OOA/UseCases/UC-###.md 산출. OOA, Use Case, SSD, system
  operation 작업 시 사용.
---
...
| **Typical Courses of Events** | **기본 성공 경로** — 번호 단계, 한 단계 = Actor 행동 **또는** 시스템 반응 하나 · 옆에 `FR-###` / `NFR-###` |
...
| **시나리오 ID 요약** | `UC-##-S01` … 목록 + 대응 **SSD 섹션명** (`SSD-UC-##-S##`) |
...
(mermaid sequenceDiagram — actor → :System system event)

| System Event | System Operation | Parameters | FR/NFR |
```

**SSD diagram 예시 (mermaid)**
...
- [ ] 입력: `docs/OOA/01-System-Requirements.md` 존재
- [ ] Typical 최소 `S01` + Alternative/Exceptional은 FR 근거 있을 때만
- [ ] 시나리오 ID마다 UC 내 **SSD 섹션 1개** · Actor·`:System` 존재
- [ ] UC 단계 순서 = SSD 메시지 순서 · 모든 단계에 FR/NFR
- [ ] Black-Box · UI-Free · 출처 없는 시나리오 없음

## 완료 보고

`01-System-Requirements` · UC 건수 · SSD(시나리오) 건수 · 경로 목록 · 미작성 SD(OOD) 링크 안내
